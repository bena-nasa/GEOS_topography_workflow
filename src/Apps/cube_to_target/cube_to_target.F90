...
!  integer :: jmax_segments_coarse,jall_coarse,ncube_coarse
  ...
  real(r8) :: mean_target, mean_intermediate
  ...
  namelist /topoparams/ &
       ...
       ! normalize mean height to intermidiate cube
       ladjustmean, &
       !
...
!--jtb
  ...
  ! in the 'exchange grid'
   ...
   jall_anticipated = ncube*ncube*12*10 !anticipated number of weights (can be tweaked) !orig bmaa
   !jall_anticipated = ncube*ncube*12*12 !anticipated number of weights (can be tweaked)
   ...
   if (nrank == 1) then
     ...
     if (da_min_target==0) then !bug with MPAS files
        ...
        jmax_segments = jmax_segments*10 !bma added this line because jmax_segments was too small used 3 for production
     end if
...
!--jtb
  ...
  ! adjust mean height
  IF (ladjustmean) THEN
     write(*,*)'bma adjusting mean'
     mean_target = 0.0d0
     mean_intermediate = 0.0d0
     do i=1,size(terr)
        mean_intermediate=mean_intermediate+terr(i)
     enddo
     mean_intermediate=mean_intermediate/dble(size(terr))
     do i=1,ntarget
        mean_target = mean_target + terr_target(i)
     enddo
     mean_target = mean_target/dble(ntarget)
     terr_target=terr_target*real(mean_intermediate/mean_target,kind=4)
  END IF
  !
...
SUBROUTINE overlap_weights(weights_lgr_index_all,weights_eul_index_all,weights_all,&
  ...
  jall = 1
  DO i=1,ntarget
    if (mod(i,50000)==0) write(*,*)"bmaa i ",i,DBLE(i)/DBLE(ntarget)
    ! bmaa commented out the next line because too much printing and replaced with new line
    !if (MOD(i,50)==0)call progress_bar("# ", i, DBLE(50*i)/DBLE(ntarget))
    ! bmaa the line below is a new line
    !if (MOD(i,ntarget/100)==0)call progress_bar("# ", i, DBLE(100*i)/DBLE(ntarget))
    !
    !---------------------------------------------------          
    !
